package component
{
	...
	public class RichTextArea extends Sprite
	{
		...
		public function RichTextArea($w:int, $h:int)
		{
			...
			_txtInfo={cursorIndex: null, firstPartLength: null, lastPartLength: null}
		...
		private function change():void
		{
			...
			if ($_textInfo.cursorIndex > _txtInfo.cursorIndex)
				checkTxtFormat(_txtInfo.cursorIndex, $_textInfo.cursorIndex);
		...
		private function checkTxtFormat($beginIndex:int, $endIndex:int):void
		{
			...
			while ($_gap--)
			{
				...
				if ($_str.indexOf(SEPARATOR) != -1)
				{
					...
					//trace($_arr[0],$_arr[1]);
					$_txtInfo={iconStr: $_arr[0], iconType: $_arr[1], iconUrl: $_iconUrl, index: $_index}
					if ($_iconUrl == null)
					{
		...
		{
			_txtInfo={cursorIndex: _textField.caretIndex, firstPartLength: _textField.caretIndex, lastPartLength: _textField.length - _textField.caretIndex}
		}

		private function getTextInfo():Object
		{
			var $_obj:Object={cursorIndex: _textField.caretIndex, firstPartLength: _textField.caretIndex, lastPartLength: _textField.length - _textField.caretIndex};
			return $_obj;
		}
		...
		private function getInfoFormXML($str:String):Object
		{
			...
			var $_info:Object={index: -1, iconStr: "", iconUrl: "", iconType: ""}
		...
		private function addIcon($iconInfo:Object):void
		{
			...
			{
				_spriteMap.push({item: $item, iconStr: $iconInfo.iconStr, iconType: $iconInfo.iconType, iconUrl: $iconInfo.iconUrl, index: $iconInfo.index, textFormat: null, status: STATUS_INIT});
				//trace($item.width);
			...
			else if ($iconInfo.iconType == TYPE_PNG)
			{
				addJpg($iconInfo, $_onItemLoaded);
			}
		...
		private function addPng($info:Object, $onComplete:Function=null):void
		{
			addJpg($info,$onComplete);
		}
		...
		public function set richText($str:String):void
		{
			...
			if ($str == null || $str == "" || _configXML == null)
				return;
